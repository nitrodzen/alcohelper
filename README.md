...
- Мини-админка `/admin/allowlist` для управления приглашениями из аккаунта `nitrodzen@gmail.com` без перезапуска приложения.
...
В `.env` обязательно замени `POSTGRES_PASSWORD`, продублируй тот же пароль в `DATABASE_URL`, задай длинный `NEXTAUTH_SECRET` и добавь `OPENAI_API_KEY`. Email и домены из env работают как резервный allowlist; адреса, добавленные администратором через портал, хранятся в PostgreSQL и применяются сразу. Если оба списка пусты, создание новых аккаунтов закрыто; существующие пользователи продолжают входить.
